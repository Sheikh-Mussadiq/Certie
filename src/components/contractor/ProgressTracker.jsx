...
const ProgressTracker = ({ currentStep = 'building-type' }) => {
  ...
  const getStepStatus = (stepId) => {
    ...
    const stepOrder = ['service-details', 'additional-info', 'time-date', 'contact', 'complete']
        ...
        {steps.map((step, index) => {
          ...
          <div key={step.id} className="flex flex-col items-center w-full">
            <div className="flex items-center justify-center mb-2 relative w-full">
              ...
              {/* Circle */}
              <motion.div 
                className={`w-7 h-7 rounded-full flex items-center justify-center z-10 transition-colors duration-300 ${
              ...
              {index < steps.length - 1 && (
                <div 
                  className={`h-[2px] absolute left-1/2 w-full transition-colors duration-300 ${
                    status.status === 'complete' || (status.status === 'current' && status.progress >= 0.5)
                      ? 'bg-primary-orange' 
                      : 'bg-grey-outline'
                  }`}
                />
