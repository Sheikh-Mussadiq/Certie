  ...
  const logbooks = [
    {
      ...
      description: 'Enrich your CRM data by integrating with Clearbit for better customer insights and lead generation.',
      isActive: true
    },
    {
      id: 2,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-primary-grey">Streamline work, save your time and growth easier</p>
        </div>
        ...
        {['Active', 'In Active'].map(tab => (
          <button
            ...
            className={`py-2 relative ${
              activeTab === tab ? 'text-primary-orange' : 'text-primary-grey hover:text-primary-black'
            }`}
          >
            {tab}
            {activeTab === tab && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-lg bg-grey-fill flex items-center justify-center">
                  <img src={logbook.icon} alt={logbook.name} className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="font-medium text-primary-black">{logbook.name}</h3>
                  <p className="text-sm text-primary-grey">{logbook.frequency}</p>
                </div>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" className="sr-only peer" defaultChecked={logbook.isActive} />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-orange"></div>
              </label>
            </div>
            
            <p className="text-xs text-primary-grey mb-4">{logbook.regulation}</p>
            <p className="text-sm text-primary-grey mb-4">{logbook.description}</p>
        ...
        {activeTab === 'Active' && (
          <div className="border-2 border-dashed border-grey-outline rounded-lg p-6 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="font-medium text-primary-black mb-1">Create Custom LogBook</h3>
            <p className="text-sm text-primary-grey mb-4">Add a New logbook tailored to your specific</p>
          </div>
        )}
      </div>
    </div>
