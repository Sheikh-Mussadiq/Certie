  ...
  return (
    <div className="bg-white rounded-xl p-6">
      <div className="flex items-center mb-6">
        <div className="p-2 rounded-lg bg-grey-fill/50 border border-grey-outline mr-3">
          <WatchLater fontSize="medium" className="text-primary-grey" />
        </div>
        <h2 className="text-xl font-semibold text-secondary-black">
          Outstanding Issues
        </h2>
        <span className="ml-2 bg-grey-fill/50 text-secondary-black text-sm font-semibold px-2.5 py-1 rounded-full border border-grey-outline">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {outstandingIssues.map((issue, index) => (
            <div
              key={index}
              className="flex justify-between items-center p-4 rounded-lg bg-grey-fill/50 border border-grey-outline"
            >
              <div className="flex flex-col justify-between items-start space-y-2">
                <div className="flex items-center space-x-2">
                  <p className="font-semibold">{issue.logbookName}</p>
                  <span className="text-xs text-primary-grey">
                    {formatDate(issue.performed_at)}
                  </span>
                </div>

                <p className="text-sm text-gray-600 truncate w-96">
              ...
              <span className="text-sm px-3 py-1 rounded-md bg-red-100 text-red-700 border border-red-200">
        ...
        <div className="text-center p-8 text-gray-500 bg-grey-fill/30 rounded-lg">
