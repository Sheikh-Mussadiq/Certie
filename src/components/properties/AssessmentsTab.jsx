...
import { MoreHorizontal, Plus, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react'
...
const AssessmentsTab = () => {
  ...
  const stats = [
    { label: 'Total Assessments', value: '04', change: '+10.2%', isPositive: true },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white rounded-lg border border-grey-outline p-4">
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-grey-fill border-b border-grey-outline">
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Assessment Type</th>
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Date Completed</th>
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Next Due</th>
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Status</th>
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Performed By</th>
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Document</th>
              <th className="text-left py-4 px-6 text-sm font-medium text-primary-grey">Reminders</th>
            </tr>
          </thead>
          <tbody>
            {assessments.map((assessment, index) => (
              <tr key={index} className="border-b border-grey-outline last:border-0">
                <td className="py-4 px-6 text-sm">{assessment.type}</td>
                <td className="py-4 px-6 text-sm">{assessment.dateCompleted}</td>
                <td className="py-4 px-6 text-sm">{assessment.nextDue}</td>
                <td className="py-4 px-6">
                  <span className={`px-3 py-1 text-xs font-medium rounded-full ${getStatusColor(assessment.status)}`}>
                    {assessment.status}
                  </span>
                </td>
                <td className="py-4 px-6">
                ...
                <td className="py-4 px-6">
                  <div className="w-12 h-6 rounded-full bg-grey-fill relative cursor-pointer">
                    <div className={`absolute w-5 h-5 rounded-full top-0.5 transition-all ${
                      assessment.reminder ? 'bg-primary-orange right-0.5' : 'bg-gray-400 left-0.5'
                    }`} />
                  </div>
                </td>
              </tr>
            ...
            {[1, 2, 3, '...', 6].map((page, index) => (
              <button
                ...
                className={`w-8 h-8 rounded flex items-center justify-center text-sm ${
                  page === 1 ? 'bg-primary-orange text-white' : 'text-primary-black hover:bg-grey-fill'
                }`}
              >
                {page}
              </button>
